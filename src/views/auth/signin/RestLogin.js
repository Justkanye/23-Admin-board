            ...
            <Formik
                ...
                onSubmit={async (values, { setErrors, setStatus, setSubmitting }) => {
                    ...
                    if (existingUserEmail.length) {
                        const currentUser = existingUserEmail[0];
                        if (currentUser.password === values.password) {
                            dispatcher({
                                type: LOGIN,
                                payload: { user: { email: currentUser.email, username: currentUser.username, gender: currentUser.gender } }
                            });
                            ...
                            if (scriptedRef.current) {
                                ...
                                setErrors({ submit: 'Incorrect credentials' });
                                setSubmitting(false);
                            }
                        }
                    } else {
                        if (scriptedRef.current) {
                            setStatus({ success: false });
                            setErrors({ submit: 'User does not exist!' });
                            setSubmitting(false);
                        }
                    }
                    // try {
